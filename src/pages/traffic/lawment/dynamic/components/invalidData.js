...
import CarDetail from './invalidData/modal';
...
@connect(({ system, DynamicLaw, loading, Site }) => ({
  Site,
  system,
  DynamicLaw,
...
class InvalidData extends PureComponent {
  ...
  showDetailModal = record => {
    this.setState({ previewCode: record.previewCode });
  ...
  getList = params => {
    ...
    dispatch({
      type: 'DynamicLaw/getBusInvalidData',
      payload: params,
    });
  };
  ...
  importClick = () => {
    ...
    dispatch({
      type: 'DynamicLaw/exportBusInvalid',
  ...
  render() {
    ...
    const { modalVisible, previewCode } = this.state;
