...
import InvalidDataModal from '../../modal/InvalidDataModal';
...
class DetailModal extends PureComponent {
      ...
      <div className={themeStyle.modalHeader}>
        <div className={themeStyle.title}>
          ...
          <span className={themeStyle.time}>
            {moment(busStaticData.previewTime).format('YYYY-M-D HH:mm:ss')}
          </span>
        </div>
        <div className={themeStyle.titleRight}>
          <Button type="primary" style={{ marginRight: 8 }} onClick={() => this.verifyPassClick()}>
            审核通过
          </Button>
          <Button type="danger" onClick={() => this.handleInvalidVisible(true)}>
            数据作废
          </Button>
        </div>
      </div>
  ...
  verifyPassClick = () => {
    ...
    dispatch({
      type: 'LawmentStatic/examinationPassed',
      payload: {
        id: detailID,
      },
      callback: () => {
  ...
  render() {
    ...
    return (
      <Fragment>
        ...
        {invalidVisible ? (
          <InvalidDataModal {...parentMethods} modalVisible={invalidVisible} />
        ) : null}
      </Fragment>
    );
  }
