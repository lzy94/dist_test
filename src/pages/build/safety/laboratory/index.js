...
@connect(({ BuildSafetyLaboratory, loading }) => ({
  BuildSafetyLaboratory,
  loading: loading.models.BuildSafetyLaboratory,
}))
@Form.create()
class TableList extends PureComponent {
  ...
  delUtil = ids => {
    ...
    dispatch({
      type: 'BuildSafetyLaboratory/remove',
      payload: { ids },
      callback: () => {
  ...
  getList = parmas => {
    const { dispatch } = this.props;
    dispatch({
      type: 'BuildSafetyLaboratory/fetch',
      payload: parmas,
    });
  };
  ...
  showDetailModal = detailID => {
    const { dispatch } = this.props;
    dispatch({
      type: 'BuildSafetyLaboratory/detail',
      payload: detailID,
      callback: detail => {
        this.setState(
          {
  ...
  handleSearch = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      const values = {
        ...
        construcUnit: fieldsValue.jldw,
        principalName: fieldsValue.jgfzr,
      };
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={16}>
          ...
          <Col md={5} sm={24}>
            <FormItem>
              {getFieldDecorator('jldw')(<Input addonBefore="监理单位" placeholder="请输入" />)}
            </FormItem>
          </Col>
          <Col md={5} sm={24}>
            <FormItem>
              {getFieldDecorator('jgfzr')(<Input addonBefore="机构负责人" placeholder="请输入" />)}
            </FormItem>
          </Col>
          <Col md={4} sm={24}>
