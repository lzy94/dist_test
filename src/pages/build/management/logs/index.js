...
class TableList extends PureComponent {
  ...
  delUtil = ids => {
    ...
    dispatch({
      type: 'BuildManagementlogs/remove',
      payload: { ids },
      callback: () => {
  ...
  getList = parmas => {
    const { dispatch } = this.props;
    dispatch({
      type: 'BuildManagementlogs/fetch',
      payload: parmas,
    });
  };
  ...
  showDetailModal = detailID => {
    const { dispatch } = this.props;
    dispatch({
      type: 'BuildManagementlogs/detail',
      payload: detailID,
      callback: detail => {
        this.setState(
          {
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={16}>
          <Col md={5} sm={24}>
            <FormItem>
              {getFieldDecorator('jlr')(<Input addonBefore="记录人" placeholder="请输入" />)}
            </FormItem>
          </Col>
          <Col md={5} sm={24}>
