...
@connect(({ MaritimeDangerFlag, loading }) => ({
  MaritimeDangerFlag,
  loading: loading.models.MaritimeDangerFlag,
}))
@Form.create()
class TableList extends PureComponent {
  ...
  delUtil = (ids, callback) => {
    const { dispatch } = this.props;
    dispatch({
      type: 'MaritimeDangerFlag/remove',
      payload: { ids },
      callback: () => {
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={16}>
          <Col md={6} sm={24}>
            <FormItem>
              {getFieldDecorator('shipName')(<Input addonBefore="船舶名称" placeholder="请输入" />)}
            </FormItem>
          </Col>
          <Col md={6} sm={24}>
            <FormItem>
