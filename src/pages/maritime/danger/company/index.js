...
@connect(({ MaritimeDangerCompany, loading }) => ({
  MaritimeDangerCompany,
  loading: loading.models.MaritimeDangerCompany,
}))
@Form.create()
class TableList extends PureComponent {
  ...
  getList = params => {
    ...
    dispatch({
      type: 'MaritimeDangerCompany/fetch',
      payload: params,
    });
  };
  ...
  delUtil = (ids, callback) => {
    const { dispatch } = this.props;
    dispatch({
      type: 'MaritimeDangerCompany/remove',
      payload: { ids },
      callback: () => {
  ...
  handleSearch = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      const values = {
        ...fieldsValue,
        createTime: fieldsValue.time && moment(fieldsValue.time).format('YYYY-MM-DD'),
      };
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={16}>
          <Col md={5} sm={24}>
            <FormItem>
              {getFieldDecorator('companyName')(
                <Input addonBefore="单位名称" placeholder="请输入" />,
              )}
            </FormItem>
          </Col>
          <Col md={5} sm={24}>
          ...
          <Col md={5} sm={24}>
            <FormItem>
              {getFieldDecorator('phone')(<Input addonBefore="联系方式" placeholder="请输入" />)}
            </FormItem>
          </Col>
          <Col md={4} sm={24}>
