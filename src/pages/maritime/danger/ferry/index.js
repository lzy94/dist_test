...
@connect(({ MaritimeDangerFerry, loading }) => ({
  MaritimeDangerFerry,
  loading: loading.models.MaritimeDangerFerry,
}))
@Form.create()
class TableList extends PureComponent {
  ...
  getList = params => {
    ...
    dispatch({
      type: 'MaritimeDangerFerry/fetch',
      payload: params,
    });
  };
  ...
  delUtil = (ids, callback) => {
    const { dispatch } = this.props;
    dispatch({
      type: 'MaritimeDangerFerry/remove',
      payload: { ids },
      callback: () => {
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={16}>
          <Col md={6} sm={24}>
            <FormItem>
              {getFieldDecorator('ferryName')(
                <Input addonBefore="渡口名称" placeholder="请输入" />,
              )}
            </FormItem>
          </Col>
          <Col md={6} sm={24}>
            <FormItem>
              {getFieldDecorator('authority')(
                <Input addonBefore="批准单位" placeholder="请输入" />,
              )}
            </FormItem>
          </Col>
          <Col md={6} sm={24}>
            <FormItem>
              {getFieldDecorator('township')(<Input addonBefore="所属乡镇" placeholder="请输入" />)}
            </FormItem>
          </Col>
          <Col md={6} sm={24}>
